...
#
# Runs on the board, like every other script in here. jug2 builds its own image
...
# The kubelet resolves a bare `jug-console:tag` to this, so the image is built
# under the name it will be pulled by.
IMAGE=docker.io/library/jug-console
...
kube -n jug rollout status deployment/jug-console --timeout=180s
...
curl -sf --max-time 20 -o /dev/null -w "  localhost:80 -> HTTP %{http_code}\n" http://localhost/ \
  || echo "  ingress did not answer — sudo k3s kubectl -n jug logs deploy/jug-console"
kube -n jug get pods -o wide

echo
echo "Rollback:            sudo k3s kubectl -n jug rollout undo deployment/jug-console"
