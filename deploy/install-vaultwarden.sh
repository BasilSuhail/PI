...
echo "==> Config"
if kube -n jug get configmap "$CONFIG" >/dev/null 2>&1; then
  echo "  already present — leaving it alone"
  echo "  registration is currently: $(kube -n jug get configmap "$CONFIG" -o jsonpath='{.data.SIGNUPS_ALLOWED}')"
else
  ...
  kube -n jug create configmap "$CONFIG" \
...
kube -n jug rollout status deployment/vaultwarden --timeout=300s

echo "==> Waiting for its tailnet name"
HOST=""
for _ in $(seq 1 60); do
  HOST="$(kube -n jug get ingress vault -o jsonpath='{.status.loadBalancer.ingress[0].hostname}' 2>/dev/null || true)"
  [ -n "$HOST" ] && break
  sleep 5
done
...
if [ -n "$HOST" ]; then
  ...
  echo "  Watch: sudo k3s kubectl -n jug get ingress vault -w" >&2
fi
...
Next, in this order:
  ...
  3. Close registration, so nobody else can make an account:
       ...
       sudo k3s kubectl -n jug patch configmap vaultwarden-config \
         --type merge -p '{"data":{"SIGNUPS_ALLOWED":"false"}}'
       sudo k3s kubectl -n jug rollout restart deployment/vaultwarden
