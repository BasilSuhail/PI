...
- [ ] **`tailscale up --hostname=jug2`.** Tailscale is installed on jug2 but logged out. Nothing on jug2 is reachable from a phone until this runs.
- [ ] **cgroup flag on jug1.** Its `mem_limit`s are unenforced today.
...
- [ ] Image jug1's SD card. It is boot-only and holds no data, but its loss means the board will not start.
