...
- **No registry.** jug2 runs containerd without Docker and cannot build its own image. It is built on the Mac for `linux/arm64`, saved, copied over, and imported with `k3s ctr images import` — which writes to the `k8s.io` containerd namespace, the one the kubelet reads. The tag is the commit: reusing a tag under `imagePullPolicy: IfNotPresent` leaves the old image in place and reports success.
